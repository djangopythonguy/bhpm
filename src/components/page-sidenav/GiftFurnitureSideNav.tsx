...
const NavbarRoot = styled(BazarCard)<{
  isfixed: boolean;
  bgcolor: any;
  colors: any;
}>(({ isfixed, bgcolor, colors }) => ({
  position: "relative",
  height: "auto",
  borderRadius: "8px",
  zIndex: 10,
...
const StyledList = styled(FlexBox)<{ colors: any }>(({ theme, colors }) => ({
  transition: "all 0.2s",
  padding: "4px 20px",
  alignItems: "center",
  ...
  "&:hover": {
    color: colors.main,
    "& .listCircle": { background: colors.main },
  },
}));

const BorderBox = styled(Box)<{ colors: any }>(({ colors }) => ({
  display: "flex",
  alignItems: "flex-end",
...
const GiftFurnitureSideNav: FC<GrocerySidenavProps> = (props) => {
  ...
  return (
    <SimpleBar style={{ maxHeight: sidebarHeight, boxShadow: shadows[1] }}>
